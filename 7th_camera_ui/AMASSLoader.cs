...
public class AMASSLoader : MonoBehaviour
{
    ...
    public bool ABLoopEnabled => abLoopEnabled;
    public int FrameA => frameA;
    public int FrameB => frameB;
    ...
    {
        if (motionList == null || motionList.motions.Count == 0) { Debug.LogError("MotionList가 없습니다."); return; }
        if (index < 0 || index >= motionList.motions.Count) { Debug.LogError($"잘못된 모션 인덱스: {index}"); return; }
        ...
        if (!File.Exists(path)) { Debug.LogError("파일을 찾을 수 없습니다: " + path); return; }

        byte[] bytes = File.ReadAllBytes(path);
        if (bytes.Length == 0) { Debug.LogError("파일이 비어있습니다: " + data.fileName); return; }
        ...
        if (poses.shape[0] == 0) { Debug.LogError("프레임 수가 0입니다: " + data.fileName); return; }
    ...
    public void SetFrameA(int frame)
    {
        frameA = Mathf.Clamp(frame, 0, TotalFrames - 1);
        if (frameA > frameB) frameB = frameA;
    }

    public void SetFrameB(int frame)
    {
        frameB = Mathf.Clamp(frame, 0, TotalFrames - 1);
        if (frameB < frameA) frameA = frameB;
    }

    public void SetABLoop(bool enabled)
    {
        abLoopEnabled = enabled;
        if (enabled && (CurrentFrame < frameA || CurrentFrame > frameB))
            SeekFrame(frameA);
    }
